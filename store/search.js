...
export const getters = {
  ...
  blogData(state) {
    ...
    if (state.searchData && state.searchData.article) {
      return state.searchData.article
    }
  },
...
export const mutations = {
  ...
  setAutocompleteNotVisible(state) {
    ...
    // console.log('autocomplete not visible', state.autocompleteVisible)
  },
      ...
      this.$nacelle.data.connector
        ...
        .then(res => {
          if (res && res.data) {
            ...
            const products = res.data.product.filter((item) => {
              return item.pimSyncSource === 'shopify' 
                && item.handle !== 'blendjet-2-influencer-kit'
                && item.handle !== 'blendjet-2-press-kit'
                && item.handle !== 'youtube-blendjet-2'
            })
