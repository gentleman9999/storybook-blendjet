<template>
  <div class="recipe">
    ...
    <div v-if="page.fields.ingredients" class="recipe__ingredients ingredients">
      ...
      <div class="ingredients__list-wrapper">
        <ul class="ingredients__list">
          ...
          >
            {{ ingredient.fields.serving}} <em class="ingredients__list-item-em">{{ ingredient.fields.ingredient.fields.title }}</em>
          </li>
        </ul>
        <button role="btn" class="ingredients__nutrition-btn btn btn--outline-purple" @click="$modal.show('nutrition-modal')">Nutrition Facts</button>
      </div>
    </div>
    ...
    <div class="recipe__facebook facebook">
      ...
      <div class="facebook__text">For more <span class="blendjet-receipes-facebook"><a href="/recipes">BlendJet recipes</a></span> like this, and to share your own, join our BlendJet Recipes Facebook Group!</div>
  ...
  async asyncData ({params}) {
    ...
    .then(async (res) => {
      ...
      for (const ingredientSet of data.fields.ingredients) {
        const ingredient = await client.getEntry(ingredientSet.sys.id)
        .then(async (res) => {
...
// }
  ...
  .ingredients {
    ...
    &__labels-label {
      ...
      &:nth-child(1), &:nth-child(7) { opacity: .3; }
      &:nth-child(2), &:nth-child(6) { opacity: .5; }
      &:nth-child(3), &:nth-child(5) { opacity: .7; }
    }
